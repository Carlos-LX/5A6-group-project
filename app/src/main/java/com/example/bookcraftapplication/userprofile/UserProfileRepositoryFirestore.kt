...
class UserProfileRepositoryFirestore(val db: FirebaseFirestore) : UserProfileRepository {
    ...
    override suspend fun getProfile(name: String): Flow<ProfileData> =
        callbackFlow {
    ...
    override suspend fun getProfiles(): Flow<List<ProfileData>> =
        callbackFlow {
            // Listen for changes on entire collection
            val subscription =
                dbUserProfiles.addSnapshotListener { snapshot, error ->
                    if (error != null) {
                        // An error occurred
                        println("Listen failed: $error")
                        return@addSnapshotListener
